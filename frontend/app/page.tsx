...
// 🛑 REMOVED: import SteamersPanel from '@/components/SteamersPanel';
        ...
        <div className="max-w-7xl mx-auto px-4 pt-4 pb-2">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    {/* 🚨 REMOVED TOGGLE SWITCH. JUST SHOWING SCANNER STATUS. */}
                                                    ...
                                                    {event.selections?.map((runner: any) => {
                                                        ...
                                                        if (runner.exchange.lay > 1.0) {
                                                            ...
                                                            if (bestBookPrice > 1.0) {
                                                                ...
                                                                const edgeROI = ((bestBookPrice / runner.exchange.lay) - 1) * 100;
                                                                
                                                                // COLOR LOGIC: Green if > 0, Yellow if > -0.01, else Gray
                                                                let textColor = 'text-slate-500';
                                                                if (edgeROI > 0.01) textColor = 'text-emerald-400';
                                                                else if (edgeROI > -0.01) textColor = 'text-amber-400';
                                                                
                                                                const sign = edgeROI > 0 ? '+' : '';

                                                                // ALWAYS SHOW TEXT (Even if negative)
                                                                valueText = (
                                                                     <div className="flex flex-col mt-1">
                                                                        <span className="text-[10px] text-slate-500 font-mono">
                                                                             Best: <span className="text-slate-300 font-bold">{bestBookName} {bestBookPrice.toFixed(2)}</span>
                                                                        </span>
                                                                        <div className="flex gap-2 text-[9px] font-mono leading-tight">
                                                                            <span className={textColor}>ROI: {sign}{edgeROI.toFixed(1)}%</span>
                                                                        </div>
                                                                     </div>
                                                                );
